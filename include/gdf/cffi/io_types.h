...
typedef struct {
	 ...
	 */
	const int		*num_cols;							// Out: number of columns
	const int		*num_rows;							// Out: number of rows
	...
	const boolean 	delim_whitespace	= false;		// use white space as the delimiter
	const boolean	skipinitialspace	= false;		// Skip spaces after delimiter
	...
	const boolean	skip_blank_lines	= true;			// whether or not to ignore blank lines

	const char		*prefix				= NULL;			// if there is no header or names, attend this to the column ID as the name
	const boolean	mangle_dupe_cols	= false;		// if true: duplicate columns will be specified as ‘X’, ‘X.1’, …’X.N’, rather than ‘X’…’X’  - CSV reader does not utilize column name for processing
	...
	const boolen	parse_dates			= true;			// parse date field into date32 or date64.  If false then date fields are saved as a string. Specifying a date dtype overrides this
	const boolean	infer_datetime_format = true;		// try and determine the date format
	const boolean	dayfirst			= false;		// is the first value the day?  DD/MM  versus MM/DD
	...
	const boolean	doublequote			= true;			// whether to treat two quotes as a quote in a string or empty
