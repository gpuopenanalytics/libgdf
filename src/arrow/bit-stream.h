...
/// bytes in one read (e.g. encoded int).
    ...
    public:
        ...
        void SetGpuBatchMetadata(int num_bits, int batch_size, int values_read,
                                 std::vector<int> & unpack32InputOffsets, std::vector<int> & unpack32OutputOffsets,
                                 std::vector<int> & remainderInputOffsets, std::vector<int> & remainderBitOffsets,
                                 std::vector<int> & remainderSetSize, std::vector<int> & remainderOutputOffsets);
    ...
    inline void BitReader::SetGpuBatchMetadata(int num_bits, int batch_size, int values_read,
                                               std::vector<int> & unpack32InputOffsets, std::vector<int> & unpack32OutputOffsets,
                                               std::vector<int> & remainderInputOffsets, std::vector<int> & remainderBitOffsets,
                                               std::vector<int> & remainderSetSize, std::vector<int> & remainderOutputOffsets) {
...
//	  } else {  // WSM i think the logic for our purposes is the same here even if sizeof(T) != 4 because we are not copying into an intermediate buffer
...
//		  		  for (int j = 0; j < num_loops; ++j){
//		  			  unpack32InputOffsets.push_back(byte_offset);
//		  			  unpack32OutputOffsets.push_back(i);
//
//		  			  i += 32;
//		  			  byte_offset += 32 * num_bits / 8;
//		  		  }
...
//	  }
    ...
    inline int BitReader::GetBatch(int num_bits, T* v, int batch_size) {
      ...
      if (ARROW_PREDICT_FALSE(bit_offset != 0)) {
        for (; i < batch_size && bit_offset != 0; ++i) {
          ::arrow::detail::GetValue_(num_bits, &v[i], max_bytes, buffer, &bit_offset, &byte_offset,
                            &buffered_values);
        }
      }
      ...
      if (sizeof(T) == 4) {
        ...
        std::cout<<"unpack32 num_bits "<<num_bits<<std::endl;
        std::cout<<"unpack32 byte_offset "<<byte_offset<<std::endl;

        int num_unpacked =
                ::arrow::internal::unpack32(reinterpret_cast<const uint32_t*>(buffer + byte_offset),
                                   reinterpret_cast<uint32_t*>(v + i), batch_size - i, num_bits);
        i += num_unpacked;
        byte_offset += num_unpacked * num_bits / 8;
        std::cout<<"unpack32 num_unpacked "<<num_unpacked<<std::endl;
        std::cout<<"unpack32 new byte_offset "<<byte_offset<<std::endl;
      } else {
        const int buffer_size = 1024;
        uint32_t unpack_buffer[buffer_size];
        while (i < batch_size) {
...
#pragma warning(pop)
      ...
      std::cout<<"detail::GetValue_ i "<<i<<std::endl;
      std::cout<<"detail::GetValue_ batch_size "<<batch_size<<std::endl;
      std::cout<<"detail::GetValue_ max_bytes "<<max_bytes<<std::endl;
      std::cout<<"detail::GetValue_ bit_offset "<<bit_offset<<std::endl;
      std::cout<<"detail::GetValue_ byte_offset "<<byte_offset<<std::endl;
      for (; i < batch_size; ++i) {
