...
#define ARROW_CHECK(condition)                           \
  (condition) ? 0                                        \
              : gdf::arrow::detail::CerrLog(ARROW_FATAL)                                \
...
__host__ __device__ static inline uint64_t TrailingBits(uint64_t v, int num_bits) {
...
template <typename T>
__host__ __device__ inline void GetValue_(int num_bits, T* v, int max_bytes, const uint8_t* buffer,
...
OutputIterator gpu_expand(InputIterator1 first1,
...
struct unpack10_32_functor : public thrust::binary_function<uint8_t, int, uint32_t>
{
...
struct remainder_functor : public thrust::unary_function<Int4, int> {
    ...
    uint8_t* buffer;
    int* ptr_output; 
    remainder_functor (int max_bytes, int num_bits, uint8_t* buffer, int* ptr_output)
    ...
    __host__ __device__ int operator () (Int4 tuple) {
        ...
        for (; i < batch_size; ++i) {
            detail::GetValue_(num_bits, &ptr_output[i], max_bytes, (uint8_t *)buffer, &bit_offset, &byte_offset, &buffered_values);
        }
        return 0;
    }
};  
...
int decode_using_cpu(const uint8_t *buffer,
    ...
    thrust::host_vector<int> d_input_offset(input_offset);
    thrust::host_vector<int> d_output_offset(output_offset);

    thrust::transform(  thrust::host,
                        thrust::make_permutation_iterator(d_buffer.begin(), d_input_offset.begin()),
                        thrust::make_permutation_iterator(d_buffer.end(), d_input_offset.end()),
    ...
    auto zip_iterator_begin = thrust::make_zip_iterator(thrust::make_tuple(d_remainder_bit_offsets.begin(), d_remainder_input_offsets.begin(), d_remainder_output_offsets.begin(), d_remainder_setsize.begin()));
    auto zip_iterator_end = thrust::make_zip_iterator(thrust::make_tuple(d_remainder_bit_offsets.end(), d_remainder_input_offsets.end(), d_remainder_output_offsets.end(), d_remainder_setsize.end()));
...
int decode_using_gpu(const uint8_t *buffer,
    ...
    int bit_group_size = input_offset.size() / bit_pack_count;
    thrust::device_vector<int> d_output_offset(output_offset);
    std::cout << "bit_pack_count: " << bit_pack_count << std::endl;
    std::cout << "bit_group_size: " << bit_group_size << std::endl;
    
    // Misaligned address in CUDA problem: example: d_buffer.begin() + 5
    
    // https://stackoverflow.com/questions/37323053/misaligned-address-in-cuda
    for(size_t index = 0; index < bit_pack_count; index++) {
        std::vector<int> input_offset2;
        for(size_t i = 0; i < bit_group_size; i++) {
            input_offset2.push_back(input_offset[i + index * ( bit_group_size)] - input_offset[ index * bit_group_size]);
            std::cout << "input-offset: " << input_offset2[i] << std::endl;
        }
        thrust::device_vector<int> d_input_offset(input_offset2);
        thrust::device_vector<uint8_t> d_buffer2(buffer + input_offset[ index * bit_group_size] , buffer + buffer_len);
        
        thrust::transform(  thrust::cuda::par,
                            thrust::make_permutation_iterator(d_buffer2.begin(), d_input_offset.begin()),
                            thrust::make_permutation_iterator(d_buffer2.end(), d_input_offset.end()),
                            thrust::make_permutation_iterator(d_output.begin() + index * ( bit_group_size), d_output_offset.begin() + index * ( bit_group_size)),
                            thrust::make_discard_iterator(), 
                            unpack10_32_functor()
                        );
    }
    ...
    thrust::transform(
        ...
        zip_iterator_end,
        thrust::make_discard_iterator(),
        remainder_functor(max_bytes, num_bits, d_buffer.data().get(), d_output.data().get())
    );
